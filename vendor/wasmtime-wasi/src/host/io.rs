    ...
    T: WasiView,
{
    ...
    async fn blocking_write_and_flush(
        ...
        // Chunk writes to ≤4096 bytes (wasi spec limit per call)
        let mut offset = 0;
        while offset < bytes.len() {
            let end = (offset + 4096).min(bytes.len());
            self.table()
                .get_mut(&stream)?
                .blocking_write_and_flush(bytes[offset..end].to_vec().into())
                .await?;
            offset = end;
        }
        Ok(())
    ...
    async fn blocking_write_zeroes_and_flush(
        ...
        while remaining > 0 {
            ...
            self.table()
                .get_mut(&stream)?
                .blocking_write_zeroes_and_flush(chunk as usize)
                .await?;
            remaining -= chunk;
        }
        Ok(())
